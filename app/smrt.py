...
class Node:
    ...
    def __str__(self):
        return f"Node: {self.station_name} ({self.station_code} on line {self.station_line})"
...
def generate_graph(data):
    ...
    for station_code, station_info in data.items():
        ...
        if station_code == "STC":
            adjacent_stations.append(Node("SE1", data["SE1"]["name"], station_info["line"]))
            adjacent_stations.append(Node("SE5", data["SE5"]["name"], station_info["line"]))
            adjacent_stations.append(Node("SW1", data["SW1"]["name"], station_info["line"]))
            adjacent_stations.append(Node("SW8", data["SW8"]["name"], station_info["line"]))

        # PTC is connected to PE1 and PE8 && PW1 and PW7
        elif station_code == "PTC":
            adjacent_stations.append(Node("PE1", data["PE1"]["name"], station_info["line"]))
            adjacent_stations.append(Node("PE7", data["PE7"]["name"], station_info["line"]))
            adjacent_stations.append(Node("PW1", data["PW1"]["name"], station_info["line"]))
            adjacent_stations.append(Node("PW7", data["PW7"]["name"], station_info["line"]))

        else:
            number = station_info["number"]
            line = station_info["line"]

            prev_station_code = line + str(number - 1)
            if prev_station_code in data:
                prev_station_name = data[prev_station_code]["name"]
                adjacent_stations.append(Node(prev_station_code, prev_station_name, line))
            ...
            if next_station_code in data:
                ...
                adjacent_stations.append(Node(next_station_code, next_station_name, line))

            if station_code == "PW1" or station_code == "PW7" or station_code == "PE1" or station_code == "PE7":
                adjacent_stations.append(Node("PTC", data["PTC"]["name"], line))

            if station_code == "SW1" or station_code == "SW8" or station_code == "SE1" or station_code == "SE5":
                adjacent_stations.append(Node("STC", data["STC"]["name"], line))

            # CG is connected to CG1
            # CG is the same station as EW4 and so connected to EW3 and EW5
            if station_code == "CG":
                adjacent_stations.append(Node("EW3", data["EW3"]["name"], line))
                adjacent_stations.append(Node("EW5", data["EW5"]["name"], line))

            if station_code == "CG1":
                adjacent_stations.append(Node("CG", data["CG"]["name"], line))

            # CC4 to CE1 and back
            if station_code == "CC4":
                adjacent_stations.append(Node("CE1", data["CE1"]["name"], line))

            if station_code == "CE1":
                adjacent_stations.append(Node("CC4", data["CC4"]["name"], line))

            # BP6 to BP5, BP7 AND BP13
            if station_code == "BP6":
                adjacent_stations.append(Node("BP13", data["BP13"]["name"], line))

            if station_code == "BP13":
                adjacent_stations.append(Node("BP6", data["BP6"]["name"], line))  

        AL[station_code] = adjacent_stations
        CODE_TO_NODE_MAP[station_code] = Node(station_code, station_name, station_info["line"])
...
def bfs_check(station, data, visited):
    ...
    branched_range = None
    while queue:
        station, remaining_hop_count = queue.popleft()
        ...
        for station in adjacent_stations:
            ...
            if next_station_code not in visited:
                visited.append(next_station_code)
                queue.append((station, remaining_hop_count - 1))
    ...
    if branched_range and len(branched_range) > 1:
        branched_range.sort()
        result.append(branched_range)

    return result

def clean_range(ranges):
    cleaned = []

    for raw_range in ranges:
        both_connections_in = False
        connection_branches = None

        for connections in BRANCH_CONNECTIONS:
            if connections[0] in raw_range and connections[1] in raw_range:
                both_connections_in = True
                
                if connections[0] == "CE1":
                    connection_branches = BRANCHES["CE"]
                else:
                    connection_branches = BRANCHES["CG"]
