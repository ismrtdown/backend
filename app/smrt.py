...
def generate_graph(data):
    ...
    for station_code, station_info in data.items():
        ...
        # PTC is connected to PE1 and PE8 && PW1 and PW7
            ...
            if station_code == "CG":
                adjacent_stations.append(Node("EW3", data["EW3"]["name"]))
                adjacent_stations.append(Node("EW5", data["EW5"]["name"]))

            if station_code == "CG1":
                adjacent_stations.append(Node("CG", data["CG"]["name"]))

            # CC4 to CE1 and back
            if station_code == "CC4":
                adjacent_stations.append(Node("CE1", data["CE1"]["name"]))

            if station_code == "CE1":
                adjacent_stations.append(Node("CC4", data["CC4"]["name"]))
...
def bfs_check(current_station_code, data):
    queue = deque()
    queue.append((current_station_code, MARGIN)) # Tuple of (station_code, remaining_hop_count)

    visited = []
    visited.append(current_station_code)

    result = []
    while queue:
        station_code, remaining_hop_count = queue.popleft()

        if data[station_code] >= OFFICIALLY_DELAYED_THRESHOLD:
            remaining_hop_count = MARGIN
            result.append(station_code)
        ...
        for station in adjacent_stations:
            ...
            if next_station_code not in visited:
                ...
                queue.append((next_station_code, remaining_hop_count - 1))
